...
document.addEventListener('DOMContentLoaded', function() {
    const aoPoloCheckbox = document.getElementById('aoPoloCheckbox');
    const categoryCheckboxes = document.querySelectorAll('.sidebar-filter input[type="checkbox"]');
    const productItems = document.querySelectorAll('.product-item');

    // Hàm để ẩn/hiện sản phẩm dựa trên danh mục được chọn
    ...
    // Lắng nghe sự kiện thay đổi trên tất cả các checkbox phân loại
    categoryCheckboxes.forEach(checkbox => {
        checkbox.addEventListener('change', filterProducts);
    });

    // Cập nhật giá trị ban đầu cho Áo Polo
    // Giả sử khi tải trang, bạn muốn hiển thị tất cả nếu không có gì được chọn
    // Hoặc chỉ Áo Polo nếu aoPoloCheckbox mặc định được checked trong HTML.
    // Nếu bạn muốn mặc định chỉ hiện Áo Polo khi vào trang này, bạn có thể:
    // 1. Trong HTML, set aoPoloCheckbox là checked.
    // 2. Gọi filterProducts() một lần khi tải trang.
    
    // Đặt aoPoloCheckbox mặc định được chọn khi tải trang và lọc ngay lập tức
    if (aoPoloCheckbox) {
        aoPoloCheckbox.checked = true; // Đảm bảo Áo Polo được chọn mặc định
        filterProducts(); // Gọi hàm lọc ngay khi tải trang
    } else {
        // Nếu không tìm thấy aoPoloCheckbox, vẫn lọc để đảm bảo trạng thái chính xác
        filterProducts();
    }
});